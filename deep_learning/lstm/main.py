...
def main() -> None:
    ...
    number_of_batches = len(train_data) # this depends on the number of training samples
    EPOCHS = 2 # user define epochs
    STEPS_PER_EPOCH = number_of_batches // EPOCHS # this is fixed, don't change
    model.fit(train_dataset, validation_data=val_dataset, epochs=EPOCHS, steps_per_epoch=STEPS_PER_EPOCH)  # Adjust epochs as needed

    # Evaluate the model on the test dataset
    number_of_eval_sample = len(test_data)
