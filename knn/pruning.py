...
print(f1_score(y_train, knn_model.predict(X_train_selected) , average='weighted')) # 0.7574410198169914
print(f1_score(y_test, knn_model.predict(X_test_selected), average='weighted')) # 0.6635011641402764
...
print(f1_score(y_train, knn_model.predict(reduced_X_train) , average='weighted')) # 0.7802351566979954
print(f1_score(y_test, knn_model.predict(reduced_X_test), average='weighted')) # 0.6145187664889681
...
# weighted avg       0.63      0.63      0.61        86
...
for k in k_range:
    ...
    scores = cross_val_score(pipe, X_train_smote, y_train_smote, cv=cv, scoring='f1_micro')
...
plt.ylabel('Cross-Validation F1 micro Score')
...
# Optimal number of features: 478
#%% 
